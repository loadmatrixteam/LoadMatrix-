...
class User(db.Model):
    ...
    # Add profile image field to store base64 encoded image or URL
...
class Driver(db.Model):
    ...
    is_online = db.Column(db.Boolean, default=False)  # New field for online status
    last_location_update = db.Column(db.DateTime)  # New field for tracking location updates
    phone = db.Column(db.String(20))
...
class OrderStatus(str, Enum):
    PENDING = "pending"
    PENDING_DRIVER_SELECTION = "pending_driver_selection"  # New status for driver selection workflow
    REQUESTED = "requested"  # New status for driver selection
    ACCEPTED = "accepted"    # New status for driver acceptance
    ASSIGNED = "assigned"
    PICKED = "picked"
    DELIVERING = "delivering"
    DELIVERED = "delivered"
    CANCELLED = "cancelled"
    CANCELED = "canceled"    # Alternative spelling for consistency
